...
namespace MihaZupan.TelegramBotClients
{
    public class RateLimitedTelegramBotClient : ITelegramBotClient
    {
        ...
        {
            await RequestScheduler.YieldAsync(cancellationToken).ConfigureAwait(false);
        ...
        {
            await RequestScheduler.YieldAsync(chatId, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<Message> SendDiceAsync(ChatId chatId, bool disableNotification = false, int replyToMessageId = 0, IReplyMarkup replyMarkup = null, CancellationToken cancellationToken = default)
        {
            await RequestScheduler.YieldAsync(chatId, cancellationToken).ConfigureAwait(false);

            return await BaseClient.SendDiceAsync(chatId, disableNotification, replyToMessageId, replyMarkup, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            await RequestScheduler.YieldAsync(userId, cancellationToken).ConfigureAwait(false);
        ...
        public async Task SetChatAdministratorCustomTitleAsync(ChatId chatId, int userId, string customTitle, CancellationToken cancellationToken = default)
        {
            await RequestScheduler.YieldAsync(chatId, cancellationToken).ConfigureAwait(false);

            await BaseClient.SetChatAdministratorCustomTitleAsync(chatId, userId, customTitle, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task<BotCommand[]> GetMyCommandsAsync(CancellationToken cancellationToken = default)
        {
            ...
            return await BaseClient.GetMyCommandsAsync(cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task SetMyCommandsAsync(IEnumerable<BotCommand> commands, CancellationToken cancellationToken = default)
        {
            ...
            await BaseClient.SetMyCommandsAsync(commands, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task CreateNewAnimatedStickerSetAsync(int userId, string name, string title, InputFileStream tgsSticker, string emojis, bool isMasks = false, MaskPosition maskPosition = null, CancellationToken cancellationToken = default)
        {
            await RequestScheduler.YieldAsync(userId, cancellationToken).ConfigureAwait(false);

            await BaseClient.CreateNewAnimatedStickerSetAsync(userId, name, title, tgsSticker, emojis, isMasks, maskPosition, cancellationToken).ConfigureAwait(false);
        }

        public async Task AddAnimatedStickerToSetAsync(int userId, string name, InputFileStream tgsSticker, string emojis, MaskPosition maskPosition = null, CancellationToken cancellationToken = default)
        {
            await RequestScheduler.YieldAsync(userId, cancellationToken).ConfigureAwait(false);

            await BaseClient.AddAnimatedStickerToSetAsync(userId, name, tgsSticker, emojis, maskPosition, cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task SetStickerSetThumbAsync(string name, int userId, InputOnlineFile thumb = null, CancellationToken cancellationToken = default)
        {
            await RequestScheduler.YieldAsync(userId, cancellationToken).ConfigureAwait(false);
